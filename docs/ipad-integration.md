...
**单字节写入**：位掩码，沿用 USB CDC `SET_CONTROL_LINE_STATE` 的定义，写入立即生效：
...
**偶数长度写入（时序批，2–64 字节）**：`[状态, 延时]` 对的序列，桥在本地按精确时序执行。
状态为上表位掩码，延时单位 10ms（0–2550ms）。BLE 链路的延迟和抖动不影响步进间隔——
需要精确时序的复位序列（如 ESP32 进 bootloader）必须用批，不要逐字节发。

常用序列：

```
进 bootloader（USB-Serial-JTAG 目标）:
  00 0A 01 0A 03 00 02 00 02 0A 00 00
  （空闲 100ms → DTR 100ms → DTR+RTS → RTS → 保持 100ms → 释放）
  成功标志：数据特征收到 boot:0x21 (DOWNLOAD) 与 waiting for download

普通重启:
  02 0A 00 00     （RTS 拉低 EN 100ms → 释放）
```

批执行期间桥暂停串口数据发送（与数据写严格保序）。上电默认双线无效；
USB 串口设备拔插后线状态回到无效（不回放旧状态），需要时由客户端重设。
